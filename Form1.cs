namespace Dedicated_Host_Switch
{
    public partial class Form1 : Form
    {
        ...
        {
            public static readonly string[] Line = { "\tDedicatedHost = false,", "\tDedicatedHost = true,", "Dedicated host has been set to \"false\".", "Dedicated host has been set to \"true\"." };
            private static readonly FileInfo SaveFileInfo = new(Path.Combine(Path.GetDirectoryName(Application.ExecutablePath), "save.csv"));
            ...
            public static void LuaFileSelection()
            {
                OpenFileDialog FileSelection = new OpenFileDialog();
                FileSelection.Title = "File Selection";
                FileSelection.Filter = "Lua Source File (*.lua)|*.lua";
                if (FileSelection.ShowDialog() == DialogResult.OK)
                    LuaFilePath = FileSelection.FileName;
                File.WriteAllText(SaveFileInfo.FullName, LuaFilePath);
            }
        }
        private void Form1_Load(object sender, EventArgs e) => SaveFile.Initialise();
        private void btnTrue_Click(object sender, EventArgs e)
        {
            SaveFile.LuaContent[11] = SaveFile.Line[1];
            SaveFile.LuaSave();
        }

        private void btnFalse_Click(object sender, EventArgs e)
        {
            SaveFile.LuaContent[11] = SaveFile.Line[0];
            SaveFile.LuaSave();
        }

        private void btnToggle_Click(object sender, EventArgs e)
        {
            if (SaveFile.LuaContent[11] == SaveFile.Line[0])
                SaveFile.LuaContent[11] = SaveFile.Line[1];
            else
                SaveFile.LuaContent[11] = SaveFile.Line[0];
        }

        private void btnChangeFile_Click(object sender, EventArgs e) => SaveFile.LuaFileSelection();
